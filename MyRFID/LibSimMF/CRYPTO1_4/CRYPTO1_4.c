//=====================================================================================================================
//
//	Matlab wrapper of 32-bit Crypto-1 cipher function
//
//	The Matlab interface for this function will be defined as following: 
//	
//	should have the same size.
//	should all be 32bit unsigned integer
//	[ks, newstate] = CRYPTO1_4(state, input, fdbk)
//	-- ks, newstate, state, input should have the same size	
//	-- newstate, state are 64bit unsigned int. (48bit actually)
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs,const mxArray *prhs[])
{
	...
	U32 *input, *ks, fdbk, i;
	U64 *state, *newstate, tmp;

	//check the number of input and output
	if (nrhs != 3)
	{
		mexErrMsgTxt("Function needs 3 inputs: [ks, newstate] = CRYPTO1_4(state, input, fdbk)");
	}
	if (nlhs != 2)
	{
		mexErrMsgTxt("Function needs 2 outputs: [ks, newstate] = CRYPTO1_4(state, input, fdbk)");
	}
	...
	if (ndim1 != 2 || ndim2 != 2)
	{
		mexErrMsgTxt("Function first and second input should be a 1-D array of numbers: [ks, newstate] = CRYPTO1_4(state, input, fdbk)");
	}
	...
	{
		mexErrMsgTxt("Function first and second input should have the same size: [ks, newstate] = CRYPTO1_4(state, input, fdbk)");
	}

	//check if state is 64 bit unsigned integer
	if(mxGetClassID(prhs[0]) != mxUINT64_CLASS )
	{
		mexErrMsgTxt("Function first input should be 64bit unsigned integer: CRYPTO1_4(state, input, fdbk)");
	}
	//check if input is 32 bit unsigned integer
	if(mxGetClassID(prhs[1]) != mxUINT32_CLASS )
	{
		mexErrMsgTxt("Function second input should be 32bit unsigned integer: CRYPTO1_4(state, input, fdbk)");
	}

	//check if fdbk is 32 bit unsigned integer
	if(mxGetClassID(prhs[2]) != mxUINT32_CLASS )
	{
		mexErrMsgTxt("Function third input should be 32bit unsigned integer scalar: CRYPTO1_4(state, input, fdbk)");
	}
	//check if fdbk is scalar
	if (ndim3 != 2 || dims3[0]*dims3[1] !=1)
	{
		mexErrMsgTxt("Function third input and second input should be a 1-D array of numbers: [ks, newstate] = CRYPTO1_4(state, input, fdbk)");
	}
