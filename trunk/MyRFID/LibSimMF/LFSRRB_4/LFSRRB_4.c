...
//	[ks, oldstate] = LFSRRB_4(state, input, fdbk)
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs,const mxArray *prhs[])
{
	...
	if (nrhs != 3)
	{
		mexErrMsgTxt("Function needs 3 inputs: [ks, oldstate] = LFSRRB_4(state, input, fdbk)");
	}
	if (nlhs != 2)
	{
		mexErrMsgTxt("Function needs 2 outputs: [ks, oldstate] = LFSRRB_4(state, input, fdbk)");
	}
	...
	if (ndim1 != 2 || ndim2 != 2)
	{
		mexErrMsgTxt("Function first and second input should be a 1-D array of numbers: [ks, oldstate] = LFSRRB_4(state, input, fdbk)");
	}
	...
	{
		mexErrMsgTxt("Function first and second input should have the same size: [ks, oldstate] = LFSRRB_4(state, input, fdbk)");
	}

	//check if state is 64 bit unsigned integer
	if(mxGetClassID(prhs[0]) != mxUINT64_CLASS )
	{
		mexErrMsgTxt("Function first input should be 64bit unsigned integer: LFSRRB_4(state, input, fdbk)");
	}
	//check if input is 32 bit unsigned integer
	if(mxGetClassID(prhs[1]) != mxUINT32_CLASS )
	{
		mexErrMsgTxt("Function second input should be 32bit unsigned integer: LFSRRB_4(state, input, fdbk)");
	}

	//check if fdbk is 32 bit unsigned integer
	if(mxGetClassID(prhs[2]) != mxUINT32_CLASS )
	{
		mexErrMsgTxt("Function third input should be 32bit unsigned integer scalar: LFSRRB_4(state, input, fdbk)");
	}
	//check if fdbk is scalar
	if (ndim3 != 2 || dims3[0]*dims3[1] !=1)
	{
		mexErrMsgTxt("Function third input and second input should be a 1-D array of numbers: [ks, oldstate] = LFSRRB_4(state, input, fdbk)");
	}
	...
	{
		tmp = state[i];
		ks[i] = LFSRRB_4bit(&tmp, input[i], fdbk);
		oldstate[i] = tmp;
	}
